...
object gossip {
  ...
  class Worker(id: Int, topology: Topology, algorithm: Algorithm, numActors: Int)  extends Actor {
      ...
      case pushSum(s, w) =>
          ...
          if (kill & convPercent > Math.abs((this.s + s)/(this.w + w) - avg)/avg)
          {
             // Simulate restart of a node by resetting s and w
             this.s = id+1
             this.w = 1
             termCounter = 0
             kill = false
  ...
  class Master(topology: Topology, algorithm: Algorithm, numActors: Int) extends Actor {
    ...
    val termActors = new ListBuffer[Int]()
    val message: Int = 10
    var terminalCount: Int = numActors
    var startTime:Long = 0
    var convergedCount: Int = 0
    if(isFailure) terminalCount -=failureCount
    ...
    def receive = {
      ...
      case converged(id) =>
        ...
        convergedActors += id
        var n: Int = 0 
        if (isFailure && kill && convPercent < convergedCount/numActors)
        {
              n = Random.nextInt(numActors)
            while (convergedActors.contains(n))
              n = Random.nextInt(numActors)
            context.actorSelection(n.toString())!terminate
            kill = false
        ...
        if (convergedCount == terminalCount)
        {
          ...
          println("[" + convergedCount + "]" + "Convergence time is: " + time_diff.millis)
          exit
        }
  ...
  def main(args: Array[String]) {
    ...
    System.exit(1);
      ...
      if (args.length > 3)
      {
        convPercent = args(3).toFloat
        if(convPercent > 1) convPercent = 0.5
      }
